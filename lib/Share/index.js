...
/**
 * @api {put} /share/:id/published Publish Share
 * @apiName Publish
 * @apiGroup Share
 *
...
app.put('/:id/returned'
  , lookup(Share)
  , app.middleware.canGrade(Share)
  , app.actions.returnInstance);

app.put('/:id/redo'
  , lookup(Share)
  , app.middleware.canGrade(Share)
  , app.actions.redoInstance);
...
app.put('/:id'
  ...
  , app.middleware.updateInstances()
  , app.actions.save);
...
Share.schema.when('pre:add', hooks.removeChannelsIfDraft());

//recursive object generators
Share.schema.when('pre:validate', hooks.generateId());

Share.schema.when('post:add', hooks.aggregateChannel('add'));

// undraft changes status so it must run before change status
Share.schema.when('pre:change:displayName', hooks.undraft());

Share.schema.when('pre:change:status', hooks.checkDraftChannel());

Share.schema.when('pre:change:channels', function(share, next) {
  console.log('change channels', share.channels);
  next();
})

Share.schema.when('post:change:_object.0.points.scaled', hooks.aggregateChannel('update'));
Share.schema.when('post:change:status', hooks.aggregateChannel('update'));
Share.schema.when('post:change:channels', hooks.aggregateChannel('update'));
Share.schema.when('post:remove', hooks.aggregateChannel('remove'));
...
Share.schema.when('pre:add', 'pre:change:status', hooks.denyPendingAndUndenyActive());
Share.schema.when('pre:add', 'pre:change:status', hooks.setPublishedAt());


Share.schema.when('pre:add', 'pre:change:status', hooks.setQueuedAt());

Share.schema.when('pre:add', 'pre:change:status', hooks.setInstanceVerb());

Share.schema.when('pre:add', hooks.dispatchObjectType());

Share.schema.when('post:add', hooks.createAllInstances());
Share.schema.when('post:change:status', hooks.createAllInstances());
Share.schema.when('post:change:status', hooks.createStatusUpdate());

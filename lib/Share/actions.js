...
exports.to = function(req, res, next) {
  ...
  console.log('context', channels, context)
...
var likes = po(
  ...
  function _shares(likes) {
    ...
    return shares.find({_id: {$in: shareIds}}).then(function(shares) {
      return likes.map(function(edge) {
        var share = _.find(shares, function(share) {
          return share._id.toString() === edge.dest;
        });
        share.likedAt = edge.createdAt;
        return share;
      })
    })
  }
...
exports.unlike = function(req, res, next)  {
  ...
  if (likeIdx === -1)
    return next(errors.Client('You cannot unlike a share that you didnt liked.'));

...
exports.updateShare = function (req, res, next) {
  ...
  req.share.save((err, share) => {
    if (err) {
      console.log('Error saving share:', req.share)
      return next(err)
    }
...
exports.newActivity = function (req, res, next) {
  req.me
    .createSection({displayName: 'Untitled Activity'})
...
exports.setMaxPoints = function (req, res, next) {
  var qid = req.param('questionId')
  var question = req.share.object.find(qid)

  question.points.max = req.body.max

  updateShare(req.share, function (err) {
    if (err) return next(err)
    res.status(200).send()
  })
}
...
exports.scrape = function (req, res, next) {
  scrape(req.body.url, function (err, data) {
    if (err) return next(errors
      .Client('Invalid URL')
      .error('invalid', 'url', req.body.url, 'Invalid URL'))
...
exports.quickLink = function (req, res, next) {
  scrape(req.body.url, (err, scraped) => {
    const share = req.me.createSection({
      displayName: scraped.displayName || 'Untitled'
    })

    share.object.attachments.push(scraped)
    share
      .save(err => {
        if (err) return next(err)
        res.ok()

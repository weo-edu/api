...
exports.event = function(ev) {
  return function(req, res, next) {
    req.share.createStatus(ev, req.me.toKey()).save()
    next()
  }
}
...
exports.notify = function(ev) {
  return function(req, res, next) {
    var user = ev === 'pinned'
      ? (req.share.forked ? req.share.forked.actor : req.share.actor)
      : req.share.actor

    // Don't notify about our own actions
    if(user.id !== req.me.id)
      req.share.createNotification(user, ev, req.me.toKey()).save()
...
exports.deleteLikes = function(req, res, next) {
  ...
  edges.remove({dest: req.share._id.toString()}).then(function() {
    console.log('likes removed for share:', req.share.id)
  })
  next()
}

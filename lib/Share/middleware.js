    ...
    User
      ...
      .then(function (user) {
        ...
        var shareGroups = share
          .contexts
          .filter(ctx => ctx.descriptor.id !== 'me')
          .map(function (access) {
  ...
  return function (req, res, next) {
    ...
    req.share
      ...
      .exec((err, instances) => {
        ...
        let inst = _.find(instances, inst => inst.contextIds.indexOf(context) !== -1)

        if(! inst) {
          lock('instance_' + user.id, lockDone => {
          ...
          openInstance(inst, done)
        }

        function openInstance (inst, done) {
...
exports.copy = function () {
  return function (req, res, next) {
    ...
    delete old.originalImage

    old.channels = []
    old.contexts = []
    old.fork = old.fork || !!(old._forked[0] && old._forked[0].board)
...
exports.updateForked = function (prop, inc) {
  return function (req, res, next) {
    if (req.share.fork) {
      ...
      lock(req.share.forked.id, function (done) {
...
exports.sendTo = function (groupType) {
  return function (req, res, next) {
    ...
    if(groupType === 'class') {
      if(!getProp('preferences.onboard.assign_share', req.me)) {
        req.me.setPreference('onboard.assign_share', true)
        req.me.save()
      }
    }
...
exports.decrementPinCount = function (req, res, next) {
  var user = req.me
  user.pinCount -= 1
  user.save(next)
}

exports.updateUserPins = function (req, res, next) {
  var user = req.me
  var share = req.share
  user.pinCount += 1
  user.recentPins.unshift(share.toKey())
  while (user.recentPins.length > 5) {
    user.recentPins.pop()
  }
  user.save(next)
}

exports.updateUserAssigns = function (req, res, next) {
  var user = req.me
  var share = req.share
  user.recentAssigns.unshift(share.toKey())
  while (user.recentAssigns.length > 5) {
    user.recentAssigns.pop()
  }
  user.save(next)
}

exports.quickLink = function (req, res, next) {
  scrape(req.body.url, (err, scraped) => {
    if (err) return next(err)
    scraped.originalContent = req.body.url

    if (scraped.objectType === 'assignment') {
      req.share = req.me.createAssignment({
        displayName: scraped.displayName,
        description: scraped.description
      })
      req.share.object = scraped
    } else {
      req.share = req.me.createSection({
        displayName: scraped.displayName || 'Untitled'
      })

      req.share.withMe(req.me)
      req.share.object.attachments.push(scraped)
    }

...
exports.addUserTo = function(req, res, next) {
  _.remove(req.body.to , function(to) {
    return to.board === req.auth.id;
  });

  var userTo = {
    board: req.auth.id,
    allow: _.union.apply(null, _.map(req.body.to, function(to) {
      return to.allow;
    })),
    deny: _.union.apply(null, _.map(req.body.to, function(to) {
      return to.deny;
    }))[0]
  };
  req.body.to.push(userTo);
  next();
}

...
exports.updateInstances = function() {
  return function(req, res, next) {
    next();
    ...
    updateInstanceQueue.push(function(queueCb) {
      req.share.findInstances().exec(function(err, instances) {
        if(err) return queueCb(err);

        var tree = req.share.object.toJSON();
        async.each(instances, function(inst, cb) {
          var data = inst.instanceData();
          var instProps = req.share.instanceProperties({context: inst.contextIds});
          inst.set(instProps);

...
var ShareSchema = module.exports = new Schema({
  ...
  instances: selfLink.embed(function() {
    ...
    {property: 'score', replace: true, as: 'pointsScaled', root: true},
  ...
  likersLength: {type: Number, default: 0},
  ...
  commonCore: {type: Boolean, default: false}
...
ShareSchema.method('createNotification', function(recipient, statusName, actor) {
  var User = mongoose.model('User')
  var channels = [User.path(actor, 'activities')]
...
ShareSchema.method('contextsForUser', function(user) {
  ...
  return this.contexts.filter(function(context) {
    var id = context.descriptor.id
    return id === 'public' || groupIds.indexOf(id) !== -1
  })
})
...
ShareSchema.method('trashChannel', function() {
  var User = mongoose.model('User')
  return User.path(this.actor, 'trash')
})

ShareSchema.method('sendToTrash', function() {
  this.channels = [this.trashChannel()]
  this.contexts = this.contexts.filter(function (context) {
...
ShareSchema.method('isTrashed', function () {
  return this.channels.length === 1 && this.channels[0].indexOf('trash') >= 0
})
...
ShareSchema.static('isComment', function(share) {
...
ShareSchema.method('unpublish', function () {
  this.contexts = [this.contexts[this.contexts.length - 1]]
  this.published = false
})

ShareSchema.static('board', function (share) {
  var context = share.contexts[1]
  return context && context.descriptor.id !== 'me' && context.descriptor
})

ShareSchema.method('board', function() {
  var Share = mongoose.model('Share')
  return Share.board(this)
})

...
var ShareSchema = module.exports = new Schema({
  ...
  instances: selfLink.embed(function() {
    ...
    {property: 'publishedAt', replace: true, root: true, type: Date}
  ]),
  /**
...
ShareSchema.path('contexts').validate(function() {
...
ShareSchema.method('sendToDrafts', function() {
  ...
  this.channels = [User.path(this.actor, 'drafts')];
});
...
ShareSchema.static('isSheet', function(share) {
  return share._object && share._object[0].objectType === 'section' && share.shareType === 'share';
});

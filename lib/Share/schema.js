...
ShareSchema.static('findByIdAndPopulate', function(id, cb) {
...
ShareSchema.method('canEdit', function(user) {
  // You can only edit share (not shareInstances) and
  // only if you are the author of that share
  return this.shareType === 'share' && this.isOwner(user);
});
...
ShareSchema.method('withGroup', function(model, opts) {
  ...
  if (opts.channel)
    this.addChannel(Group.path(parent, 'board'));
});
...
ShareSchema.Profile = require('./Profile/schema');
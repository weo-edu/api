...
ShareSchema.method('createEvent', function(statusName) {
  var User = mongoose.model('User');
  var channel = User.path(this.actor, 'activities');
  var evt = this.createChild('status', {channels: [channel]});
  evt.set({actor: this.actor.toJSON()});
...
ShareSchema.path('contexts').validate(function(val) {
  if (val.length === 1 && this.isSheet() && !this.isDraft())
...
ShareSchema.method('sendToClasses', function() {
  var Group = mongoose.model('Group');
  this.channels = this.contexts.filter(function(context) {
    // XXX this should not be necessary
    return context.descriptor.id !== 'public';
  }).map(function(context) {
    return Group.path(context.descriptor, 'board');
  });
});

ShareSchema.method('isPublished', function() {
  return ! (this.isDraft() || this.isTemporary());
});

ShareSchema.method('isDraft', function() {
  return this.channels.indexOf(this.draftChannel()) !== -1;
});

 ...
 * Pre save/remove
 */
Schema.pre('save', function addPublicContext(next) {
...
Schema.pre('save', function addDescription(next) {
  if (this.isSheet()) {
    ...
    if (post) {
      // strip markdown
      this.description = this.originalDescription || summarizeMarkdown(post.originalContent);
    } else {
...
Schema.pre('save', function addImage(next) {
  if (this.isSheet()) {
    var image = _.find(this.object.attachments, function(attachment) {
...
Schema.post('save', function createTurnInEvent(share, next) {
  if(share.isInstance() && share.$status < status.turnedIn && share.status >= status.turnedIn) {
    share.createEvent('turned in').save(next);
  } else
    next();
});

Schema.post('save', function createPublishEvent(share, next) {
  if(share.isSheet() && share.$wasDraft && share.isPublished()) {
    share.createEvent('published').save(next);
  } else
    next();
});

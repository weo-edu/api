...
exports.broadcast = function(evt) {
  return function(share, next) {
    // Cache these here so we don't have to
    // generate them each time send() is
    // called
    ...
    if (evt === 'change' && !_.isEqual(share.previous('channels'), share.channels)) {
      send('add', _.difference(share.channels, share.previous('channels')));
      send('remove', _.difference(share.previous('channels'), share.channels));
      send('change', _.intersection(share.previous('channels', share.channels)));
    } else {
      send(evt, share.channels);
    }
...
exports.aggregateChannel = function(type) {
  return function(share, next) {
    ...
    async.each(typeChannelPairs, function(pair, cb) {
      channel.withModel(pair.channel, function(err, parent) {
        function done(err) {
          parent.done();
          cb(err);
        }

        if (err || !parent) return done(err);
        ...
        if(! model || !model.selfLink) {
          console.log('failed to locate selflink', pair.channel, leaf);
          return done(null);
        }
...
exports.removeChannelsIfDraft = function() {
  return function(share, next) {
...
exports.checkDraftChannel = function() {
  return function(share, next) {
    if (share.isSheet() && share.isQueued()) {
      share.sendToDrafts();
    } else if (share.isSheet() && share.previous('status') === 'pending') {
      share.sendToClasses();
    } else if (share.isSheet() && share.isDraft()) {
      share.channels = [];

...
exports.createProfileShare = function(adverbMatch) {
  return function(profileEvt, next) {
    if (adverbMatch && profileEvt.adverb !== adverbMatch)
      return next && next();
    Share.create('profile');
    share.object.displayName = profileEvt.type;
    share.actor = user.toKey();
    share.channels = [user.path('activities')];

    // add all user groups to context
    Group.activeOf(user).done(function(err, groups) {

...
Schema.pre('save', function (next) {
  if (!(this.channels || this.channels.length) && this.isNew && this.isSheet()) {
    this.sendToDrafts()
  }
  next()
})
...
Schema.pre('save', function addDescription (next) {
  if (this.isSheet()) {
    ...
    if (post) {
      // strip markdown
      var el = cheerio.load(summarizeMarkdown(post.originalContent || ''), {decodeEntities: false})
      el('.katex-mathml').remove()
      this.description = this.originalDescription || striptags(el.html())
    } else {
...
var imageTypes = ['image', 'video']
Schema.pre('save', function addImage (next) {
  if (this.isSheet()) {
    if (!(this.originalImage && this.originalImage.url)) {
...
function broadcastFactory (evt) {
  return function broadcast (share) {
    // We have to pass this empty options object to this particular
    // .toJSON() call, otherwise mongoose will not properly serialize
    // properties named 'object' under certain circumstances
    // Such goose.
...
function aggregateFactory (evt) {
  return function aggregate (share, next) {
    if(share.wasNew
      ...
      || share.$pointsScaled !== share.get('score')) {
      var types = {push: [], remove: [], update: []}

      if (evt === 'save') {
        if (share.wasNew) {
          types.push = share.channels.slice(0)
        } else if (share.wasModified('channels')) {
    ...
    .then(function (user) {
      ...
      switch (share.object.status) {
        ...
        case 'commented':
          ...
          if (!disableCommentedOnEmails && !share.object.meta.student) {
            return mail.commentedOnActivity(to, share.object, next)
          }
          break
        case 'joined_class':
          var disableJoinedClassEmails = user.get('preferences.email.disableJoinedClass')
          if (disableJoinedClassEmails === false) {
            return mail.joinedClass(to, share.object, next)
          }
          break
        case 'turned in':
          var disableTurnedInEmails = user.get('preferences.email.disableTurnedIn')
          if (disableTurnedInEmails === false) {
            return mail.turnedInActivity(to, share.object, next)
          }
          break
        default:
          next()
...
Schema.post('create', storePrevious)

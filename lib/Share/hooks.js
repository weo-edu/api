...
Schema.pre('save', function addImage(next) {
  if (this.isSheet()) {
    var att = _.find(this.object.attachments, function(attachment) {
      return attachment.objectType === 'image'
...
Schema.post('save', function emailNotifications(share, next) {
  ...
  mongoose.model('Share').findById(share.parent.id, function(err, activity) {
    if(err) return next(err)
    if(! activity) return next()

    User.findById(notifiedId, function(err, notified) {
      if(err) return next(err)
      if(! notified) return next()
      var opts = {
        name: notified.name.givenName || notified.displayName,
        actorId: share.actor.id,
        actorName: share.actor.displayName,
        activityId: share.parent.id,
        activityName: share.parent.displayName,
        likes: activity.likersLength,
        boardName: activity.board().displayName,
        boardLink: activity.board().url
      }

      switch(share.object.status) {
        case 'liked':
          var disableLikeEmails = notified.get('preferences.email.disableLikes')
          if(! disableLikeEmails) {
            mail.likedActivity({
              name: notified.displayName,
              email: notified.email
            }, opts, next)
          }
          break
        case 'pinned':
          var disableRepinEmails = notified.get('preferences.email.disableRepins')

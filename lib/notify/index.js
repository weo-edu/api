...
function unlikedActivity (author, liker, activity, likes) {
  analytics.track({
    userId: author.id,
...
function joinedClass (owner, group, student) {
  analytics.track({
    userId: owner.id,
    event: 'Had Student Join Class',
    properties: toKey(group)
  })
  analytics.track({
    userId: student.id,
    event: 'Joined Class',

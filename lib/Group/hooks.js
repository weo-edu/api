...
Schema.pre('save', function inductOwners(next) {
  console.log('inductOwners', this.isNew, this.isModified('owners'))

  if(! (this.isNew || this.isModified('owners')))
    return next()

  var group = this
  console.log('inductOwners', group.ownerIds)
  mongoose.model('User').find()
    .where('_id').in(group.ownerIds)
    .exec(function(err, users) {
      Seq(users)
        .parEach(function(user) {
          console.log('user.joinGroup', user.id, group.id)
          var self = this
          user.joinGroup(group)
          user.save(function (err, val) {
            console.log('joinGroup cb', err, val)
            self(err, val)
          })
        })
        .seq(function() { next() })

  ...
  id: {
    ...
    validate: [
      function(entry) {
        ...
        // type is required, id is only required
        // if type is not equal to 'public'
        return entry.type && (entry.id || entry.type === 'public');
      }
...
AddressSchema.method('grant', function(type, model) {
  model = model || {};
  model = model.toAbstractKey ? model.toAbstractKey() : model;

  var entry = access.entry(type, model);

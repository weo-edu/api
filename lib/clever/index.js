...
Clever.prototype.me = function(cb) {
  ...
  request('https://api.clever.com/me')
    ...
    .end(function(err, res) {
      if(err) return cb(err);
      self.user(res.body.data, cb);
    });
};

Clever.prototype.user = function(data, cb) {
  if(! this[data.type])
    return cb(new Error('Unknown clever user type "' + data.type + '"'));

  this[data.type](data.id, cb);
};
...
Clever.prototype.teacher = function(id, cb) {
  request('https://api.clever.com/v1.1/teachers/' + id)
    .set('Authorization', 'Bearer ' + this.token)
    .end(function(err, res) {
      if(err) return cb(err);
      cb(null, res.body.data);

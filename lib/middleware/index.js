 ...
 */
middleware.lookup = function(Model, options) {
  var modelName = null;
  if (_.isString(Model)) {
    modelName = Model;
    Model = null;
  ...
  var prop = options.prop || uncapitalize(modelName);
  ...
  return function(req, res, next) {
    Model = Model || mongoose.model(modelName);
    var query = multi ? Model.find() : Model.findOne();
    query.where(key, req.param(param))
      .exec(function(err, model) {
        if(err) return next(err);
        if(! model && options['404'])
         return res.send(404, {type: Model.modelName, param: param, key: key, val: req.param(param)});
...
middleware.errorLogging = function(opts) {

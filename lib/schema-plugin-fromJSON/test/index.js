...
describe('schema-plugin-fromJSON', function() {
  ...
  it('should work when objects change position in an array', function() {
    var A = new Schema({
      str: String,
      objectType: String
    }, {discriminatorKey: 'objectType'});
    A.discriminator('b', {str2: String});
    var schema = new Schema({object: [A]});

    var Model = mongoose.model('s', schema);

    var doc = new Model({
      object: [
        {objectType: 'a', str: 'test'},
        {objectType: 'b', str: 'asdf', str2: 'fdsa'}
      ]
    });
    ...
    doc.fromJSON(json);
    expect(doc.object[0].objectType).to.equal('b');
    expect(doc.object[0].str).to.equal('asdf');
    expect(doc.object[0].str2).to.equal('fdsa');
    expect(doc.object[1].objectType).to.equal('a');
    expect(doc.object[1].str).to.equal('test');
  });

  it('should restore defaults if the sources key is undefined', function() {
    var A = new Schema({
      str: {
        type: String,
        default: 'test'
    ...
    expect(doc.str).to.equal('test');
    doc.str = 'asdf';
    expect(doc.str).to.equal('asdf');
    doc.fromJSON({str: 'fdsa'});
    expect(doc.str).to.equal('fdsa');
    doc.fromJSON({});
    expect(doc.str).to.equal('test');
  });
});
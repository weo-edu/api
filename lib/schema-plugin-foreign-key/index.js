...
var fk = module.exports = function(schema, opts) {
  ...
  function defaultTransform() {
    return {
      id: this.id,
      url: this.url || '/' + opts.model[0].toLowerCase() + opts.model.slice(1) + '/' + this.id,
  ...
  schema.static('toAbstractKey', function(group) {
    return fk.toAbstractKey(schema.statics.toKey(group));
  });
  ...
  schema.method('toAbstractKey', function() {
    return fk.toAbstractKey(this.toKey());
  });

  schema.pre('save', function(next) {
    this.$changedKey = ! _.isEqual(this.toKey(), this.$previousKey);
    next();
  });

  schema.post('init', function foreignKey(doc) {
    doc.$previousKey = doc.toKey();
  });

  schema.post('save', function foreignKey(model) {
    model.$changedKey && model.updateAllForeignKeys();
  });
  ...
  schema.method('updateAllForeignKeys', function(cb) {
    var self = this;
    cb = cb || function() {};
    cb = _.after(refs.length, cb);

    refs.forEach(function(ref) {
      self.updateForeignKey(ref.collection, ref.path, cb);
    });
    if(! refs.length) cb();
  });
  ...
  schema.method('updateForeignKey', function(Collection, path, cb) {
    ...
    if(field) {
      ...
      _.each(desc, function(val, k) {
        update[updatePath + k] = val;
      });
...
fk.toAbstractKey = function(key) {
  return _.pick(key, 'id', 'image', 'url', 'displayName');
};
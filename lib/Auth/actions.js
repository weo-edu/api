...
function login (req, res, next) {
  ...
  if (req.headers['x-mixpanel-distinctid']) {
    const distinctId = req.headers['x-mixpanel-distinctid']
    analytics.alias({previousId: distinctId, userId: req.user._id.toString()})
    analytics.flush()
  }
  ...
  analytics.track({
    userId: req.user.id,
    event: 'Logged In Self',
    properties: prep.user(req.user)
  })
...
function create (req, res, next) {
  ...
  user.save(function (err, user) {
    ...
    if (req.headers['x-mixpanel-distinctid']) {
      const distinctId = req.headers['x-mixpanel-distinctid']
      analytics.alias({previousId: distinctId, userId: user._id.toString()})
      analytics.flush()
    }

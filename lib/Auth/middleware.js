...
middleware.ifCreate = function(createMw, loginMw) {
  return function(req, res, next) {
    if (req.body.userType)
      return createMw(req, res, next);
    else
      return loginMw(req, res, next);
  };
};
...
middleware.googleCreate = function(req, res, next) {
  ...
  User.findByGoogle(req.body, function(err, user, profile) {
    if (err) return next(err);
    if (user) {
      // If the user already exists, let's just
      // log them in
      req.user = user;
      return next();
    }
...
middleware.googleLogin = function(req, res, next) {
  ...
  User.findByGoogle(req.body, function(err, user) {
    if(! user)
      return next(errors.NotFound('User not found - create account first', 'username'));

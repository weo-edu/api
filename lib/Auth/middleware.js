...
middleware.user = function(req, res, next) {
  console.log('use 3', req.access_token);
  if(req.access_token) {
    Auth.lookupToken(req.access_token, function(err, auth) {
      console.log('done 3 err', err);
      if(err) return next(err);
      req.auth = auth;
      console.log('done use 3');
      next();
    });
  } else {
    req.auth = null;
...
middleware.ifCreate = function(createMd, loginMd) {
  return function(req, res, next) {
    console.log('ifCreate', req.body.userType, !! req.body.userType);
...
middleware.googleCreate = function(req, res, next) {
  ...
  console.log('googleCreate', req.body);
  User.findByGoogle(req.body, function(err, user, profile) {
    console.log('googleCreate cb', err, user);
...
middleware.googleLogin = function(req, res, next) {
  ...
  User.findByGoogle(req.body, function(err, user) {
    console.log('googleLogin cb', err, user);
   if (user) {
      req.user = user;
      return next();
    } else {

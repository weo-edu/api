...
exports.user = function (req, res, next) {
  try {
    req.auth = req.access_token
      ? jwt.verify(req.access_token)
      : null
  } catch (err) {
    return next(err)
  }
...
passport.use(new BearerStrategy({}, function (token, done) {
  try {
    done(null, jwt.verify(token), {scope: 'all'})
  } catch (err) {
    done(err)
  }
}))
...
exports.token = function (req, res, next) {
  if(req.query && req.query.access_token) {
    ...
    req.access_token = req.cookies.authToken
  }
...
exports.googleCreateIfNew = function (req, res, next) {
  ...
  createOAuthAccount(opts, setUser(req, function (err) {
    ...
    if (!req.profile.image.isDefault) {
      var url = req.profile.image.url.split('?')[0] + '?sz=200'
      s3upload(url, function (err, url) {
        if (err) return next(err)
        Avatar.set(req.user.id, url, next)
      })
    } else {
      next(err)
    }
...
exports.facebookCreateIfNew = function (req, res, next) {
  ...
  var opts = translateFacebookProfile(req.profile, req.body.userType)
  opts.userType = req.body.userType
    ...
    facebook
      ...
      .then(function (avatar) {
        if (!avatar.data.is_silhouette) {
          s3upload(avatar.data.url, function (err, url) {
            if (err) return next(err)
            Avatar.set(req.user.id, url, next)
          })
        } else {
...
function translateFacebookProfile (profile, userType) {
...
exports.lookupByOffice365 = function (req, res, next) {
  var authContext = new AuthenticationContext(config.office365AuthorityUrl)
  var redirectUri = req.body.redirectUri
  var code = req.body.code
  ...
  authContext.acquireTokenWithAuthorizationCode(
    ...
    config.office365ClientId,
    config.office365Secret,
    function (err, resp) {
      if (err) return next(err)

      var conds = [{'auth.office365.id': resp.userId}]
      req.profile = resp

      if (resp.userId.indexOf('@') !== -1) {
        conds.push({email: resp.userId})
      }
      ...
      mongoose.model('User')
        ...
        .then(function (user) {
          ...
          if (user && !user.get('auth.office365.id')) {
            user.set('auth.office365.id', resp.userId)
            user.save(next)
          } else {
            next()
          }
...
exports.office365CreateIfNew = function (req, res, next) {
  if (req.user) return next()
  if (!req.profile) return next(new Error('office365Create requires req.profile'))

  var opts = translateOffice365Profile(req.profile, req.body.userType)
  opts.userType = req.body.userType
  opts.auth.office365.access_token = req.profile.accessToken
  opts.auth.office365.refresh_token = req.profile.refreshToken
...
function translateOffice365Profile (profile) {
  var email = profile.userId.indexOf('@') === -1
    ? profile.userId + '@weo.io'
    : profile.userId

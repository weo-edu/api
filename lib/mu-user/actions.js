...
const transformProp = curry(function(xf, path) {
  return converge(assoc(path), compose(xf, prop(path)), identity);
});
const lowercaseProp = transformProp(toLowerSafe);
...
const lowercaseUsername = evolve({username: toLowerSafe});
const lowercaseEmail = evolve({email: toLowerSafe});
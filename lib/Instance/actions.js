...
function updateAssignment (req, res, next) {
  ...
  Share.model
    .findOne({_id: ObjectId(inst._parent[0].id)})
    .exec((err, share) => {
      share.object.attachments.forEach(att => {
        if (progress[att.guid] && progress[att.guid].complete) {
          setResponse(inst, att._id, 'url', progress[att.guid].url)
          setResponse(inst, att._id, 'response', [true])
          setResponse(inst, att._id, 'score', 1)
        }
      })

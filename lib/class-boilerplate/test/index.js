...
describe('Class boilerplate', function() {
  ...
  function testGenerate(done) {
    async.waterfall([
      function(cb) {
        var username = pad(user.username.length + 6, user.username);
        Teacher.findUsernameLike(username, cb);
      },
      ...
      function(share, cb) {
        ...
        var someGraded = _(share.instances.total[0].actors).some(function(aggregation) {
          expect(!!aggregation.turnedInAt).to.be.true;
          return aggregation.pointsScaled > 0;
        });
        // test grading happens
        expect(someGraded).to.equal(true);
        Teacher.findById(user.id, cb);
      },
      function(teacher, cb) {
        expect(teacher.preferences.onboard).to.be.undefined;
        cb();
      }
    ], function(err) {

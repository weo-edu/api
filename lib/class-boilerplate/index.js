    ...
    async.series([
      function(cb) {
        async.each(students, function(student, done) {
          var username = pad(student.username.length + 6, student.username)
          User.findUsernameLike(username, function(err, username) {
            if (err) throw err
      ...
      function(cb) {
        async.each(students, function(student, done) {
          ...
          student.example = true

          student.save(function(err, doc) {
            Avatar.set(studentsStore.getAvatar(student.displayName), doc._id, function(err) {
      ...
      function(cb) {
        async.map(students, function(student, done) {
          ...
          instance.ourPost('save', function(instance, next) {
            next()
            Share.findById(instance.id, function(err, inst) {
              done(err, inst)
            })
          })

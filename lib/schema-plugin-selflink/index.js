  ...
  Schema.method('selfLink', function (property) {
    ...
    return {
      ...
      context: function (context) {
        ...
        contexts = _.uniq(contexts.concat(context))
        return this
      },
      totals: function () {
      ...
      get: function (key, actorId) {
        var keyOpts = keyOptions(key)
        return self[property].total.filter(function (total) {
          return contexts.indexOf(total.context) !== -1
        }).map(function (total) {
          return actorId
      ...
      sum: function (key, actorId) {
        var keyOpts = keyOptions(key)
        return this.get(key, actorId).reduce(function (memo, num) {
          return memo + num
        }, keyOpts.default)
      },
      hasActor: function (actorId) {
        return self[property].total.some(function (total) {
          if(contexts.indexOf(total.context) === -1)
            return false

          return _.has(total.actors, actorId)
        })
      },
...
selfLink.totalSchema = new Schema({
  context: {
    ...
    validate: [
      function (str) {
        return str === 'public' || str === 'me' || validateObjectId(str)
      }
    ]
  },
  items: {

  ...
  Schema.method('selfLink', function(property) {
    ...
    return {
      ...
      keyValue: function(property, as, share) {
        var prop = this.keyOption(as, 'root')
          ? share.get(property)
          : share.object.get(property);

        return prop || this.keyOption(as, 'default');
      },
...
selfLink.schema = new Schema({
  ...
  last: {
    actor: {
      displayName: {type: String, trusted: true},
      id: {type: String, trusted: true},
      image: {
        url: {type: String, trusted: true}
      },
      url: {type: String, trusted: true}
    },
    updatedAt: {type: Date, trusted: true},
    id: {type: String, trusted: true}
  }
}, {id: false, _id: false});

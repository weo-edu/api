  ...
  Schema.method('selfLink', function(property) {
    ...
    return {
      ...
      increment: function(key, val, actor) {
        ...
        var canonicalTotal = self[property].canonicalTotal;
        canonicalTotal[key] = canonicalTotal[key] || 0;
        canonicalTotal[key] += val;
        if (self[property].markModified)
          self[property].markdModified('canonicalTotal.' + key);

...
describe('schema-plugin-selflink', function() {
  ...
  it('should aggregate sums of numbers', function() {
    ...
    total().push(b);
    expect(total().get('num')).to.equal(1);

    var c = new B({actor: {id: 'user'}, num: 3});
    total().push(c);

    expect(total().get('num')).to.equal(4);

    total().remove(b);
    expect(total().get('num')).to.equal(3);
  });
  ...
  it('should replace', function() {
    ...
    c.num = 10;
    total().update(c);
    expect(total().get('num')).to.equal(10);
  });
  ...
  it('should replace strings', function() {
    ...
    expect(total().get('status')).to.equal('a');
    expect(total().get('status', 'user1')).to.equal('a');

    total().push(new Share({actor: {id: 'user2'}, status: 'b'}));
    expect(total().get('status')).to.equal('b');
    expect(total().get('status', 'user2')).to.equal('b');

    total().update(new Share({actor: {id: 'user3'}, status: 'c'}));
    expect(total().get('status')).to.equal('c');
    expect(total().get('status', 'user3')).to.equal('c');
  });
});

...
var stopWords = [/test/i, /activity/i, /assignment/i, /warmup/i]
...
exports.similar = function(req, res, next) {
  ...
  var query = []
    ...
    .concat(me.recentAssigns.map(function (pin) {
      return pin.displayName
    }))
    .concat(me.location)
    .join(' ')
  ...
  followingUsers(me.id)
    .then(function (edges) {
      ...
      var ids = actors.map(function (actor) {
        return actor.id
      }).map(function(id) {
          ...
          },
          {
            $project: {
              score: {
                $add: [
                  {$meta: 'textScore'},
                  {$multiply: [0.1, {$add: ['$followers', '$pinCount']}]}
                ]
              },
              displayName: 1,
              username: 1,

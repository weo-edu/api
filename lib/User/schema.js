...
var UserSchema = module.exports = new Schema({
  ...
  reputation: selfLink.embed(function() {
    return '/share?' + qs.stringify({channel: this.path('reputation')});
  }, ['points']),

  activities: selfLink.embed(function() {
    return '/share?' + qs.stringify({channel: this.path('activities')});
  })
...
UserSchema.method('createSection', function(options) {
  var share = this.createShare('section', _.extend({
    public: true
  }, options || {}));
  share.status = 'draft';
  return share;

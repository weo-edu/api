...
Schema.pre('save', function setCreateProfile(next) {
  if(getProp('gradeLevels.length', this)) {
    this.setPreference('onboard.grade_levels_set', true)
  }
  if(getProp('subjects.length', this)) {
    this.setPreference('onboard.subjects_set', true)
  }

  const subjectsSet = getProp('preferences.onboard.subjects_set', this)
  const gradeLevelsSet = getProp('preferences.onboard.grade_levels_set', this)

  if(subjectsSet && gradeLevelsSet) {
   this.setPreference('onboard.profile_set', true)
  }
  next()
})

Schema.pre('save', function setJoinedGroup(next) {
  var hasClass = (this.groups || []).some(function (group) {
    return group.groupType === 'class'
  })

  if (hasClass && !this.preferences.group_joined) {
...
Schema.post('save', function track (user, next) {
  if (!user.wasNew) return next()

  var name = user.isTeacher()

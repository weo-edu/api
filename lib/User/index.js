...
User.schema.when('pre:change:groups', function(user, next) {
  console.log('groups', user.groups, user.previous('groups'), _.isEqual(user.groups[0].toJSON(), user.previous('groups')[0]));
  next();
});

User.schema.when('pre:change', function(user, next) { console.log('pre change', user.id); next()});

...
middleware.me = function(opts) {
  ...
  return function(req, res, next) {
    if(! req.auth) {
      if(should404) return next(errors.Authentication('me middleware requires req.auth'))
      else return next()
    }
...
middleware.is = function(type) {
  return function(req, res, next) {
    if(! req.auth)
      return next(errors.Authentication('is middleware requires req.auth'))
...
middleware.isAdmin = function(req, res, next) {
  if(! req.me) return next(errors.Authentication('isAdmin middleware requires req.me'))
  if(! req.me.admin)
...
middleware.byUsernameOrEmail = function(req, res, next) {
  ...
  User.where(validations.email(username) ? 'email' : 'username', username)
    .findOne(function(err, user) {
      if(err) return next(err)
      if(! user) return next(errors.NotFound('User not found', 'username', username))

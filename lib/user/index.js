...
const hashPassword = flip(curryN(2, passwordHash.generate))(config.hash);
...
const composeDisplayName = function(name) {
 ...
 */
user.setSimilarUsernameFromEmail = evolveFrom({username: similarUsernameFromEmail});
 ...
 */
user.composeDisplayName = evolveFrom({displayName: compose(composeDisplayName, prop('name'))});
 ...
 */
user.hashPassword = evolve({password: hashPassword});

...
module.exports = function(Schema) {
  ...
  var BaseObjectSchema = new Schema({
    objectType: {
      type: String,
      required: true
    },
  ...
  BaseObjectSchema.post('init', function() {
    var self = this;
    self._objects = {};
    var object = null;
    for (var i = 0; i < this.attachments.length; i++) {
      var object = this.attachments[i];
      var keys = Object.keys(object._objects);
      for (var j = 0; j < keys.length; j++) {
        self._objects[keys[j]] = object._objects[keys[j]];
      }
    }
  });

  // objects are embedded in shares
  BaseObjectSchema.virtual('id').get(function() {
    var root = this;
    while (root.__parent) {
      root = root.parent();
    }
    return root !== this ? root._id + '-' + this._id : this._id;
  });

  var AttachableObjectSchema = BaseObjectSchema.extend({
  ...
  AttachableObjectSchema.add({
    attachments: [AttachableObjectSchema],
    replies: {
      selfLink: {
        type: String
      }
  ...
  ObjectSchema = BaseObjectSchema.extend({});
  ObjectSchema.add({
    attachments: [AttachableObjectSchema]
  });

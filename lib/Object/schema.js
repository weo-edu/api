...
ObjectSchema.method('isGradeable', function() {
  return false;
});

ObjectSchema.method('score', function() {
  var score = 0;
  if(this.isGradeable() && this.isCorrect)
    score += this.isCorrect();

  return score + this.attachments.reduce(function(memo, att) {
    return memo + att.score();
  }, 0);
});

ObjectSchema.method('totalPoints', function() {
  var total = 0;
  if(this.isGradeable())
    total++;

  return total + this.attachments.reduce(function(memo, att) {
    return memo + att.totalPoints();
  }, 0);
});
